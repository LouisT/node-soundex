...
(function(Setup) {
  ...
  Setup(function (str,scale,mysql) {
          var split = String(str).toUpperCase().replace(/[^A-Z]/g,'').split(''),
              ...
              build = [];
          build = split.map(function (letter, index, array) {
                for (var num in keys) {
          ...
          var max = (scale?((max=~~(build.length*2/(mysql?2:3.5)))>3?max:3):3);
          return split[0]+(build.join('')+(new Array(max+1).join('0'))).slice(0,max).replace(/\./g,'');
  });
